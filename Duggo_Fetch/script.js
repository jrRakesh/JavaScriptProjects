...
function getBreedFromURL(url) {
  ...
  const breedComponents = breedPart.split("-");
  if (breedComponents.length === 1) {
    return breedComponents[0];
  } else if (breedComponents.length === 2) {
    return `${breedComponents[1]} ${breedComponents[0]}`;
  }
...
async function fetchMessage(url) {
  const urlResource = await fetch(url);
  const parsedResource = await urlResource.json();
  const { message } = parsedResource;
  return message;
}
